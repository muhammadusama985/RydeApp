...
class DriverOtp extends StatefulWidget {
   DriverOtp({super.key, required this.verificationId});
...
class _DriverOtpState extends State<DriverOtp> {
  ...
  try {
    ...
    if (userCredential.user != null) {
      ...
      await FirebaseFirestore.instance.collection('VerifiedDrivers').doc(uid).set({
        'phone': phoneNumber,
        'createdAt': createdAt,
        'userId': uid,
      });
                  ...
                  child: TextButton(
                    onPressed: () {
                      Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const DrvrSignIn()));
                    },
                    child: const Text(
                      "Cancel",
                      style: TextStyle(
